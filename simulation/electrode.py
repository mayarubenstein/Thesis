...
class electrode2D:
    ...
    def __init__(self, prox, stick, theta, timestep):
        self.prox = prox
    ...
    def movingAndShaking(self, potential):
        released = []
        for particle in copy.copy(self.stuck): # make a copy of the array 
            # so that iteration doesn't messed up when release changes self.stuck
            particle.react(potential)
            if self.release(particle):
                released.append(particle)
        return released
        
    # returns True if the particle is bound to the electrode or has
    # collided with the electrode
    # returns False otherwise
    def hit(self, particle, potential):
        newPosZ = particle.pos[self.Z]
        distCutoff = numpy.max([particle.rad, self.prox])
    ...
    def Bernoulli(self, p):
        return numpy.random.binomial(1, p) == 1
    
    def release(self, particle):
        leaving = self.Bernoulli(1 - self.stick)
        if leaving:
            self.stuck.remove(particle)
            return True
        return False
    
    def rebound(self, particle):
        angle = numpy.random.uniform(low = 0, high = self.theta)
        phi = numpy.random.uniform(low = 0, high = 2* numpy.pi)
        sigma = numpy.sqrt(2*particle.diffusion*self.timestep)
        rho = numpy.sqrt(3)*numpy.abs(numpy.random.normal(0, sigma))
        xMove = rho * numpy.sin(angle) * numpy.cos(phi)
        yMove = rho * numpy.sin(angle) * numpy.sin(phi)
        zMove = rho * numpy.cos(angle)
    ...
    def interception(self, particle, newPos):
        ...
        ak = oldPos[2]
        bk = newPos[2]
        factor = (radius-ak/(bk - ak))
        displacementVector = newPos-oldPos
        return displacementVector * factor + oldPos
...
class porousElectrode:
        ...
        for p in possiblePores:
            if p.entryExitCondition(particle, newPos):
                if p.collisionChecker(particle, newPos):
                    p.entryPlacement(particle, newPos)
                    return self.ENTER
                return self.COLLIDE
        return self.NOPORE
    ...
    def movingAndShaking(self, potential):
        return self.movingAndShakingPores(potential) + self.movingAndShakingSurface(potential)
...
class pore:
    ...
    def xyIntercept(self, particle, oldPos, newPos):
        ax = oldPos[0]
        ay = oldPos[1]
        bx = newPos[0]
        by = newPos[1]
        ...
        C = (ax - cx)**2 + (ay - cy)**2 - (self.radius-particle.rad)**2
    ...
    def floorCollision(self, particle, newPos):
        floorPos = -self.depth
        interceptedPos = self.posAtZ(particle, newPos, floorPos + particle.rad)
    ...
    def wallCollision(self, particle, newPos):
        ...
        t = self.xyIntercept(particle, oldPos, newPos)
        interceptedPos = self.coordsFromT(oldPos, newPos, t)
        assert t>= 0 and t<= 1
        if interceptedPos[self.Z] <= -self.depth + particle.rad:
            return False
        ...
        for particle in copy.copy(list(self.floorStuck)):
            ...
            if self.floorRelease(particle):
                ...
                if newPos[self.Z] > 0 and self.entryExitCondition(particle, newPos): # check if exiting pore
                    ...
                    self.movingAndPlace(particle, newPos)
        return exit
        ...
        for particle in copy.copy(list(self.wallStuck)):
            ...
            if self.wallRelease(particle):
                ...
                if newPos[self.Z] > 0 and self.entryExitCondition(particle, newPos): # check if exiting pore
                    ...
                    self.movingAndPlacing(particle, newPos)
        return exit
    ...
    def movingAndShakingInterior(self, potential):
        ...
        for particle in copy.copy(list(self.moveable)):
            ...
            if newPos[self.Z] > 0 and self.entryExitCondition(particle, newPos): # if it is leaving the pore
                ...
                self.movingAndPlacing(particle)
